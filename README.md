...
基于 Python + FastAPI 后端的多 Agent 协作学术研究分析系统。5 个专业 Agent 按流水线协作，通过 OpenAI 兼容 API 对学术内容进行深度分析，前端实时展示协作过程和分析结果。

## ✨ 功能特点

- **5 Agent 流水线协作**：协调者 → 分析专员 → 评审专员 → 综合专员 → 研究顾问
- **3 种分析模式**：论文分析、文献综述、研究方案
- **实时可视化**：协作流程图、Agent 状态动画、进度条、实时日志
...
├── main.py                  # CLI 命令行入口
├── pipeline.py              # Agent 协作管线（CLI 模式使用）
├── agents/
│   ├── base.py              # Agent 基类
...
每个 Agent 的输出会作为下一个 Agent 的上下文输入，形成协作链。
...
|------|------|------|
| `/` | GET | 前端页面 |
| `/api/start-analysis` | POST | 启动分析任务（后台异步执行） |
...
| `/api/results/{type}` | GET | 获取指定类型的分析结果 |
...
GET /api/logs?since_id=0      → { logs[], last_id }
...
页面上的 **⚡ 快速测试** 振钮可以一键模拟完整的 Agent 流水线：
